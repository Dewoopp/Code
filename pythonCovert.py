...
while i < len(python_processing): # Need to use while, because list gets bigger!
    ...
    if "elif" in python_processing[i]:
        python_processing[i] = python_processing[i].replace("elif",'ELSE IF')
...
for i in range(len(python_processing)):
    if "(self, " in python_processing[i]:
            python_processing[i] = python_processing[i].replace("(self, ","(")
    if "(self):" in python_processing[i]:
            python_processing[i] = python_processing[i].replace("(self):"," ()")
    if "self" in python_processing[i]:
            python_processing[i] = python_processing[i].replace("self","THIS")
    if "__init__" in python_processing[i]:
            python_processing[i] = "    #CONSTRUCTOR\n" + python_processing[i].replace("__init__","Constructor")
    if "class " in python_processing[i]:
            python_processing[i] = "    #CONSTRUCTOR\n" + python_processing[i].replace("class ","CLASS ")
            ending_insert(i,"END CLASS")
